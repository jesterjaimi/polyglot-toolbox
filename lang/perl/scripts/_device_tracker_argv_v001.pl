...
my $job_handle = "_device_tracker_v001_";

my $num_args = $#ARGV + 1;
if ($num_args != 2) {
  my $errmsg = "\nUsage: _device_tracker_argv_v000.pl [ubr] [dest]\n";
  print "$errmsg";
  print '@ARGV {\n' . @ARGV . '}\n';
  die;
} else { ($ubr, $buffer) = @ARGV; }
...
sub _track_device_status
{
  ...
  if($buffer != "FILE" ) {
    ...
    open (URBFH > "/tmp/$p_ubr.dat") or die "STREAM BUFFER ERROR: FAILED TO OPEN FILE /tmp/$p_ubr.dat";
  }
      ...
      if ($session->login($ubr_username, $ubr_password))
      {
        ...
        if ($session->enable($ubr_password))
        {
          ...
          for(my $ix=4;$ix<=$#apinforeg;$ix++)  {
            ...
            if ($apline[0]) {
              if($buffer == "FILE" ) {
                my $mac = _format_mac_from_cmts($apline[8]);
                print URBFH "$p_ubr, $ubrcommand, $apline[0], $apline[2], $apline[7], $mac";
              } else {
