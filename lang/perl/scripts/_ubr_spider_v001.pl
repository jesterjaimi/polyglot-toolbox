...
my $job_handle = "_ubr_spider_v001_";
my $buffer = "NOBUF";
my @g_ubrList;
...
if (@ARGV) {
    my $num_args = $#ARGV + 1;
    if ($num_args != 1) {
      my $errmsg = "\nUsage: _ubr_spider_v001.pl [buffer == {'FILE'}]\n";
      print "$errmsg";
      print '@ARGV {\n' . @ARGV . '}\n';
      die;
    } elsif ($ARGV[0] != "FILE"){
      my $errmsg = "\nInvalid Agruement: ARGV[0] should == 'FILE'\n";
      print "$errmsg";
      print '@ARGV {\n' . @ARGV . '}\n';
      die;
    } else { $buffer = $ARGV[0]; }
...
@g_ubrList = _init_device_list();
 
run_threads(@g_ubrList);

if($buffer == "FILE")
{
    _bulk_load_stats(@g_ubrList);
}
...
sub _track_device {
        ...
        if ($debug) {
            ...
            system("perl /home/jbecker/scripts/_device_tracker_argv_v000.pl $p_ubr $buffer ");       
        }
        if ($tty_echo) {
